...
按照 design 中定义的升级路径（Phase 1→6）将 `oasis/aws-wrappers` 从 PHP 7.4 + PHPUnit 5.7 升级到 PHP 8.5 + PHPUnit 13，同时补齐单元测试、引入 PBT、建立覆盖率考核、完成源代码风格现代化并同步文档。
...
- [~] 4. Phase 2: PHPUnit 5.7 → 13 迁移
  ...
  - [ ] 4.2 更新 `phpunit.xml` 为 PHPUnit 13 兼容格式
  ...
  - [ ] 4.3 迁移现有集成测试到 `ut/integration/` 目录
  ...
  - [ ] 4.5 更新 `ut/ut-bootstrap.php` 确保兼容 PHPUnit 13
    - 检查并修复 bootstrap 中的 deprecation 用法
    - _Requirements: 2.4_
...
- [~] 5. Phase 3: PHP 7.4 → 8.5 + 依赖升级（含 symfony/cache 替换）
  ...
  - [ ] 5.3 执行 `composer update` 并解决依赖冲突
    - 使用 `php composer.phar update`（PHP 8.5 运行时）
    - 逐个排查并解决依赖版本冲突
    - 确认 `composer install` 无错误
    - _Requirements: 5.5_
  - [ ] 5.4 替换 `AwsConfigDataProvider` 中的 `doctrine/common` 缓存为 `symfony/cache`
    - 将 `DoctrineCacheAdapter` + `FilesystemCache` 替换为 `Psr16CacheAdapter` + `Psr16Cache` + `FilesystemAdapter`
    - 按 design 中的目标实现代码修改
    - 运行 5.1 中编写的测试确认通过（GREEN）
    - _Requirements: 5.2_
  - [ ] 5.5 修复 PHP 8.5 deprecation warnings
...
- [~] 6. Phase 4: 源代码风格现代化
  - [ ] 6.1 编写风格现代化的回归测试基线
    ...
    - 记录当前测试数量和通过状态
    - _Requirements: 6.9_
  - [ ] 6.2 对 `src/` 下全部 24 个 PHP 文件进行现代化改造
...
- [~] 7. Phase 5: PBT 引入 + 覆盖率考核机制
  - [ ] 7.1 添加 Eris PBT 库依赖
    ...
    - 确认安装成功
    - _Requirements: 7.1_
  - [ ] 7.2 编写 Property 1: Codec Round-Trip PBT 测试
  ...
  - [ ] 7.3 编写 Property 2: Typed Codec Round-Trip PBT 测试
  ...
  - [ ] 7.4 编写 Property 3: Codec Idempotence PBT 测试
  ...
  - [ ] 7.5 配置覆盖率收集与阈值检查
    - 确认 `phpunit.xml` 中 `<source>` 配置正确（已在 4.2 完成）
...
- **PHP 运行时切换**：Phase 1-2 使用 `php74` 命令（PHP 7.4），Phase 3 起切换到 `php` 命令（PHP 8.5）
...
- **Test-first 编排**：Phase 1 先写测试再验证现有代码行为；Phase 3 中 symfony/cache 替换先写测试（5.1）再改实现（5.4）
- **升级路径约束**：严格按 Phase 1→2→3→4→5→6 顺序执行，每个 Phase 完成后全量测试必须通过
- **Composer 命令**：Phase 1-2 使用 `php74 composer.phar`（或系统 composer），Phase 3 起使用 `php composer.phar`（PHP 8.5）
...
- **Design CR Q2→A**：Phase 2 一个 task（Task 4）、Phase 3 一个 task（Task 5），symfony/cache 替换包含在 Task 5 中 ✓
- **Design CR Q3→A**：Phase 4 整体一个 task（Task 6）✓
- **Design CR Q4→A**：PBT + 覆盖率合并为一个 task（Task 7）✓
- **Requirements CR Q1→A**：所有源文件都有 mock 版本单元测试 → Task 1-3 覆盖全部 20 个可测试源文件 ✓
- **Requirements CR Q2→C**：mixed 按场景判断 → Task 6.2 明确说明 ✓
- **Requirements CR Q3→A**：整个 suite 级别跳过 → Notes 中说明 `--testsuite unit` 跳过方式 ✓
- **Requirements CR Q4→C**：覆盖率执行方式灵活 → Task 7.5 和 9.3 中体现 ✓

### Q: Test-first 编排是否贯穿？

A: 是。Phase 1（Task 1-3）本身就是先写测试建立回归基线。Phase 3 中 symfony/cache 替换采用 RED→GREEN：先写测试（5.1）再改实现（5.4）。Phase 4 先确认基线（6.1）再改代码（6.2）。Phase 5 PBT 测试本身就是测试编写。

### Q: Requirements 覆盖是否完整？

A: 已检查全部 9 条 Requirement：
- Req 1（补 UT）→ Task 1-3（Phase 1 三组）
- Req 2（PHPUnit 13）→ Task 4（Phase 2）
- Req 3（集成测试迁移）→ Task 4.3, 4.4
- Req 4（PHP 8.5）→ Task 5.2, 5.5
- Req 5（依赖升级）→ Task 5.1-5.4
- Req 6（代码风格）→ Task 6
- Req 7（PBT）→ Task 7.1-7.4
- Req 8（覆盖率）→ Task 7.5
- Req 9（文档更新）→ Task 8
...
A: 是。Task 1-8 为实现 task，Task 9 为手工测试（第一个 sub-task 9.1 为 "Increment alpha tag"），Task 10 为 Code Review。符合 Release 分支的 top-level task 结构要求。

### Q: 是否存在悬空或孤立的代码？

A: 不存在。每个 task 都建立在前一个 task 的基础上：Phase 1 建立测试基线 → Phase 2 迁移测试框架 → Phase 3 升级运行时和依赖 → Phase 4 现代化源代码 → Phase 5 增强测试基础设施 → Phase 6 同步文档。每个 Phase 的 Checkpoint 确保增量验证。
...
- [内容] Requirement 追溯补全：Task 4.2 补充 Req 3 AC3（集成测试跳过粒度）引用；Task 4.3 补充 Req 3 AC3.4（profile 支持）引用及描述；Task 5.5 补充 Req 4 AC4.3/4.4（PHP 8.5 下测试通过）引用
- [内容] Code Review task 10.1 移除展开的 review checklist，改为委托给 code-reviewer sub-agent 执行（review 策略由 agent 自身定义）
...
- [x] 无 markdown 格式错误
- [x] `## Tasks` section 存在
- [x] Release spec 手工测试 top-level task（Task 9）第一个 sub-task 是 "Increment alpha tag"
- [x] 最后一个 top-level task（Task 10）是 Code Review
- [x] 实现 task（1-8）排在手工测试和 Code Review 之前
...
- [x] top-level task 按依赖关系排序（Phase 1→2→3→4→5→6→手工测试→Code Review），无循环依赖
- [x] Checkpoint 不作为独立 top-level task，而是每个 top-level task 的最后一个 sub-task
- [x] 每个 Checkpoint 包含具体验证命令和 commit 动作
- [x] Test-first 编排贯穿：Phase 1 先写测试建立基线；Phase 3 RED→GREEN（5.1→5.4）；Phase 4 先确认基线（6.1）再改代码（6.2）
...
- [x] Design CR 决策全部体现（Q1→B 三组、Q2→A 各一个 task、Q3→A 整体一个 task、Q4→A 合并一个 task）
- [x] Requirements CR 决策全部体现（Q1→A 全量 mock UT、Q2→C mixed 按场景、Q3→A suite 级别跳过、Q4→C 灵活执行）
- [x] Design 全覆盖：全部 6 个 Phase 均有对应 task，所有模块和接口均被覆盖
